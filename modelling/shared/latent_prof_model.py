...
# --------------------------------------------------
# Static panel prep shared by plug-in and full propagation
# --------------------------------------------------
...
def prepare_static_panel(
    ...
    """
    Build the static Step 3 panel:
    - filter formation years
    - compute PROF_w
    - compute sigma_raw (from plug-in sigma column, for diagnostics/reference)
    - keep only formation years with enough firms and valid observed variance

    Returns:
    - filtered firm-year panel
    - static year summary
    - year_info dict keyed by FormationYear
    """
    df = df.copy()
...
# --------------------------------------------------
# Plug-in EB logic (current approach)
# --------------------------------------------------
...
def run_empirical_bayes_by_year_plugin(
    ...
    for fy, info in year_info.items():
        ...
        theta_post_mean = lambda_i * theta_obs_valid + (1.0 - lambda_i) * mu_t
    ...
    signals_df = pd.concat(frames, ignore_index=True).sort_values(["FormationYear", "Ticker"]).reset_index(drop=True)
    year_summary_df = pd.DataFrame(year_results).sort_values("FormationYear").reset_index(drop=True)
...
def get_parquet_column_names(parquet_path: Path) -> List[str]:
    try:
        ...
        # Fallback: read entire parquet just to inspect columns
...
def load_hb_full_posteriors(
    ...
    missing = sorted(required - set(all_cols))
    if missing:
...
def run_empirical_bayes_full_propagation(
    ...
    """
    Full propagation of Step 2 HB uncertainty into Step 3 EB.

    For each selected sigma draw:
    - use draw-specific sigma_{i,t}
    - rerun EB by formation year
    - accumulate:
        * theta_post_mean
        * p_q5
        * lambda_i
    ...
    Final outputs:
    - theta_post_mean = MC mean of draw-level posterior means
    - p_q5 = MC mean of draw-level probabilities
    - theta_post_sd = sqrt(E[post_var_i] + Var_draw(theta_post_mean_draw))
    """
    ...
    # Merge selected draws onto the static panel
    merge_cols = ["Ticker", "Year"]
    merged = df[merge_cols].merge(
        hb_draw_df,
        on=["Ticker", "Year"],
        how="left",
        validate="1:1",
    ...
    draw_matrix = merged[selected_draw_cols].to_numpy(dtype=float)  # shape: (n_rows, n_draws)
    n_rows, total_draws = draw_matrix.shape

    # Running firm-year accumulators
    ...
    # Running year-level accumulators
    ...
    for j, draw_col in enumerate(selected_draw_cols):
        ...
        for fy, info in year_info.items():
            ...
            if valid.sum() < min_firms_per_year:
                # Very unlikely for HB sigma draws, but handled defensively
                continue
            ...
            lambda_i = tau2_t / (tau2_t + obs_var_i)
            theta_post_mean = lambda_i * theta_obs_v + (1.0 - lambda_i) * mu_t
            ...
            # Update year-level accumulators
        ...
        if (
            ...
            avg_per_draw = elapsed / draws_completed
            remaining = avg_per_draw * (total_draws - draws_completed)
    ...
    # Law of total variance for theta*
    ...
    # Static year fields + draw-averaged tau2
    out = out.merge(
    ...
    return out, year_summary, extra_info
...
def run_latent_prof_model(
    ...
    Supports:
    - plug-in EB (OLS or HB summary sigma)
    - HB full propagation:
        rerun EB over selected sigma draws from sigma_posteriors_full.parquet
    ...
    print(f"Static input shape after cleaning/filtering: {static_df.shape}")
    ...
    if use_full_propagation:
        if str(uncertainty_method).upper() == "OLS":
            raise ValueError(
                "use_full_propagation=True requires HB output, not OLS."
            )
        ...
        print("[mode] Running Step 3 with HB full propagation")
        ...
        print("[mode] Running Step 3 in plug-in EB mode")
        ...
        write_json(progress_json_path, progress_payload)
    ...
    config = {
        ...
        "hb_full_posterior_parquet": None if hb_full_posterior_parquet is None else str(hb_full_posterior_parquet),
        "n_available_draws_in_parquet": extra_info.get("n_available_draws_in_parquet"),
        "n_sigma_draws_requested": n_sigma_draws,
        "n_sigma_draws_used": extra_info.get("n_selected_draws"),
        "checkpoint_every_draws": checkpoint_every_draws,
        "n_rows_output": int(len(signals_df)),
        "n_years_output": int(year_summary_df["FormationYear"].nunique()) if not year_summary_df.empty else 0,
        "progress_json": str(progress_json_path),
    }
    write_json(config_path, config)

    selected_draws_payload = {
        "selected_draw_columns": extra_info.get("selected_draw_columns"),
        "n_selected_draws": extra_info.get("n_selected_draws"),
        "n_available_draws_in_parquet": extra_info.get("n_available_draws_in_parquet"),
    }
    write_json(selected_draws_path, selected_draws_payload)

    print(f"Saved firm-year output: {firm_year_path}")
    print(f"Saved year summary:     {year_summary_path}")
    print(f"Saved config:           {config_path}")
    print(f"Saved progress:         {progress_json_path}")
    print(f"Saved draw selection:   {selected_draws_path}")
    ...
    return {
        ...
        "progress_json": str(progress_json_path),
        "selected_draws_json": str(selected_draws_path),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If set, run HB full propagation using sigma_posteriors_full.parquet.",
    )
    parser.add_argument(
        "--hb_full_posterior_parquet",
        type=str,
        default=None,
        help="Path to HB sigma_posteriors_full.parquet. Required if --use_full_propagation is set.",
    )
    parser.add_argument(
        "--n_sigma_draws",
        type=int,
        default=None,
        help="How many sigma draws to use from the HB full posterior parquet. Default: all available.",
    )
    parser.add_argument(
        "--checkpoint_every_draws",
        type=int,
        default=DEFAULT_CHECKPOINT_EVERY_DRAWS,
        help="How often to print/save progress in full-propagation mode.",
    )
